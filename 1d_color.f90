...
!----------------------------------------------------------------------
  ...
  !定数
        ...
        !real(kind=8),parameter::wpe=(2*pi)*1e8,wce_t=(2*pi)*1e8,nue=(2*pi)*1e8
        real(kind=8) :: wpe,wce_t
        real(kind=8) :: nue=(2*pi)*1e8
  ...
  !----------------------------------------------------------------------
        ...
        hin=0
        hre=0
        pls=2                                !0:プラズマなし　1:非磁化プラズマ　2:磁化プラズマ
        damp=1
        if(damp==0) then
            rd=0.0
            rr=0.0
        ...
        open(01,file='energy.csv')
        !write(01,*)'wpe',',','wce',',','hy'

        do wpe=(2*pi)*1e9,(2*pi)*2.5e9,(2*pi)*0.01e9
            do wce_t=(2*pi)*1e9,(2*pi)*2.5e9,(2*pi)*0.01e9
                write(*,*)'wpe=',wpe/((2*pi)*1e9),'wce_t=',wce_t/((2*pi)*1e9)
                call setup                           !初期設定
                t=dt                                 !時間
                do n=1,nstep
                    call e_cal                         !電界の計算
